...
The common schema fields remain the editor's complete data surface: project name and description; group ID, name, and rules; and rule ID, name, source condition (`key`/`operator`/`value`), resource types, priority, and optional method. Existing source spellings and array order are preserved unless the user edits them; Compiler normalization is not written back by the editor. New IDs are deterministic collision-free values selected from the current project. Array order is source order and is never inferred from priority.
