...
The 10 steps below correspond to the phases in this skill. Each phase has its own file under `phases/` with the exact prompt to send the subagent and the human-gate behavior.
...
5. Plan review (agent + human gate).
6. Implementation (per phase, tracked in `CHECKLIST.md`).
7. Implementation review per phase (agent + human gate).
8. Commit (asked, never auto).
9. Final review (agent + human gate).
10. Refactor — only if the user opts in. Writes `docs/rpi/<feature>/REFACTOR.md`.
...
[Commit]  <--  asked, never auto
    |
    v
[...repeat implement+review+commit for each phase]
    |
    v
...
5. At the commit step (8), follow `phases/07-commit.md`: never commit; ask first and use the user's exact command.
6. After step 9, follow `phases/09-refactor.md`: ask the user whether to refactor; only proceed on yes.
...
- **rpi** (this skill) — middle ground: lightweight markdown artifacts, but review cycles after research, plan, each implementation phase, and final. Human gates are mandatory but cheap because the agent has already self-revised.
