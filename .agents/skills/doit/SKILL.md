...
Resolve models by mapping each doit stage to a shared role (see that file's **doit** phase→role table), then applying the active tier's role routing:
...
On **cursor**, walk primary → alt → cross-pool when a usage pool is maxed. On **free** / **normal**, use the documented primary/fallback or chain. Record tier, per-role model, fallbacks, and exhausted Cursor pools. Verify IDs at start (`opencode models` or Task allowlist). A model report is not verification evidence.
