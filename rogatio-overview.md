...
- **Response body:** Fetch an authorized public GET without browser credentials, then either replace the entire response body or apply bounded regex rewrites to the fetched UTF-8 body through native messaging to an explicitly started local runtime. Upstream status and headers are preserved; this is not a mock response.
